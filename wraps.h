...
struct file_descriptor {
    ...
    file_descriptor &operator=(file_descriptor other) = delete;
...
using shared_file_descriptor = std::shared_ptr<file_descriptor const>;
...
using shared_timer_fd = std::shared_ptr<timer_fd const>;
...
using shared_event_fd = std::shared_ptr<event_fd const>;
...
using shared_signal_fd = std::shared_ptr<signal_fd const>;

struct socket_wrap;

using shared_socket = std::shared_ptr<socket_wrap const>;
...
struct socket_wrap : file_descriptor {
    ...
    shared_socket accept(socket_mode mode) const;

    shared_socket accept(std::initializer_list<socket_mode> mode) const;
...
struct fd_state {
    ...
    fd_state operator|(fd_state other) const;
...
struct epoll_wrap : file_descriptor {
    using handler_t = std::function<void(fd_state, epoll_wrap &wrap)>;
    ...
    void register_fd(int fd, fd_state events, handler_t handler);

    void unregister_fd(int fd);

    void update_fd(int fd, fd_state events);

    void update_fd_handler(int fd, handler_t handler);
...
struct epoll_registration {
    ...
    epoll_registration(shared_epoll epoll, file_descriptor const &fd, fd_state state, epoll_wrap::handler_t handler);
